...
4. **署名の整合性を確認**: 画像データ、記録時刻、公開鍵、署名の組み合わせが整合しているかを確認。
...
## 6. 公開鍵の信頼モデルと構造上の前提・制約

本システムで扱う署名検証仕様における信頼モデルと技術的制約は以下の通りです：

- **現行 v2 形式（端末鍵包摂）**:
  - 画像内に含まれる端末公開鍵（X.509 SPKI 形式）を抽出し、その公開鍵で署名の整合性を検証します。
  - 現在の検証ツールは公開鍵の信頼経路やホワイトリストを持たないため、第三者が独自の鍵ペアでデータ全体を再署名し、その公開鍵も画像へ格納した場合、暗号学的な検証結果は `VERIFIED` となり得ます。
  - したがって `VERIFIED` の表示は「画像内の公開鍵・記録時刻・画像データ・署名の組み合わせが数学的に一致していること」を示すものであり、正規アプリ、正規端末、または現実の被写体・撮影事実を証明するものではありません。

---

## 7. セキュリティ上の留意事項（確認・保証できない内容）
